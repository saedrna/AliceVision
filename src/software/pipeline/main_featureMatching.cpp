...
int aliceVision_main(int argc, char** argv)
{
    ...
    std::string geometricFilterTypeName = matchingImageCollection::EGeometricFilterType_enumToString(
        matchingImageCollection::EGeometricFilterType::FUNDAMENTAL_MATRIX);
    ...
    double geometricErrorMax =
        0.0; //< the maximum reprojection error allowed for image matching with geometric validation
    ...
    optionalParams.add_options()(
        ...
        "photometricMatchingMethod,p",
        po::value<std::string>(&nearestMatchingMethod)->default_value(nearestMatchingMethod),
        ...
        "matchFromKnownCameraPoses",
        po::value<bool>(&matchFromKnownCameraPoses)->default_value(matchFromKnownCameraPoses),
        "Enable the usage of geometric information from known camera poses to guide the feature matching. "
        "If some cameras have unknown poses (so there is no geometric prior), the standard feature matching will be "
        "performed.")("knownPosesGeometricErrorMax",
                      po::value<double>(&knownPosesGeometricErrorMax)->default_value(knownPosesGeometricErrorMax),
                      "Maximum error (in pixels) allowed for features matching guided by geometric information from "
                      "known camera poses. "
                      "If set to 0 it lets the ACRansac select an optimal value.")(
        "savePutativeMatches", po::value<bool>(&savePutativeMatches)->default_value(savePutativeMatches),
        "Save putative matches.")("guidedMatching", po::value<bool>(&guidedMatching)->default_value(guidedMatching),
                                  "Use the found model to improve the pairwise correspondences.")(
        "matchFilePerImage", po::value<bool>(&matchFilePerImage)->default_value(matchFilePerImage),
        "Save matches in a separate file per image.")("distanceRatio",
                                                      po::value<float>(&distRatio)->default_value(distRatio),
        ...
        "useGridSort", po::value<bool>(&useGridSort)->default_value(useGridSort), "Use matching grid sort.")(
        "exportDebugFiles", po::value<bool>(&exportDebugFiles)->default_value(exportDebugFiles),
        "Export debug files (svg, dot).")("maxMatches",
                                          po::value<std::size_t>(&numMatchesToKeep)->default_value(numMatchesToKeep),
    ...
    std::unique_ptr<IImageCollectionMatcher> imageCollectionMatcher =
        createImageCollectionMatcher(collectionMatcherType, distRatio);
    ...
    if(geometricFilterType == EGeometricFilterType::HOMOGRAPHY_GROWING)
    {
        ...
        // This is suggested by [F.Srajer, 2016]: the matches used to be the seeds of the homographies growing are
        // chosen according to the putative matches order. This modification should improve recall.
    ...
    for(const auto& imageMatch : mapPutativesMatches)
        ALICEVISION_LOG_INFO("\t- image pair (" + std::to_string(imageMatch.first.first)
                             << ", " + std::to_string(imageMatch.first.second) + ") contains " +
                                    std::to_string(imageMatch.second.getNbAllMatches()) + " putative matches.");

    // export putative matches
    if(savePutativeMatches)
        Save(mapPutativesMatches, (fs::path(matchesFolder) / "putativeMatches").string(), fileExtension,
             matchFilePerImage, filePrefix);
    ...
    */
    ...
    ALICEVISION_LOG_INFO("Geometric filtering: using "
                         << matchingImageCollection::EGeometricFilterType_enumToString(geometricFilterType));
    ...
    switch(geometricFilterType)
    {
        ...
        case EGeometricFilterType::FUNDAMENTAL_MATRIX:
        {
            matchingImageCollection::robustModelEstimation(
                geometricMatches, &sfmData, regionPerView,
                GeometricFilterMatrix_F_AC(geometricErrorMax, maxIteration, geometricEstimator), mapPutativesMatches,
                guidedMatching);
        }
        break;
        ...
        {
            matchingImageCollection::robustModelEstimation(geometricMatches, &sfmData, regionPerView,
                                                           GeometricFilterMatrix_E_AC(geometricErrorMax, maxIteration),
                                                           mapPutativesMatches, guidedMatching);

            // perform an additional check to remove pairs with poor overlap
            std::vector<PairwiseMatches::key_type> toRemoveVec;
            for(PairwiseMatches::const_iterator iterMap = geometricMatches.begin(); iterMap != geometricMatches.end();
                ++iterMap)
            {
                const size_t putativePhotometricCount =
                    mapPutativesMatches.find(iterMap->first)->second.getNbAllMatches();
        ...
        case EGeometricFilterType::HOMOGRAPHY_MATRIX:
        {
            ...
            matchingImageCollection::robustModelEstimation(
                geometricMatches, &sfmData, regionPerView, GeometricFilterMatrix_H_AC(geometricErrorMax, maxIteration),
                mapPutativesMatches, guidedMatching, onlyGuidedMatching ? -1.0 : 0.6);
        }
        break;
        ...
        case EGeometricFilterType::HOMOGRAPHY_GROWING:
        {
            matchingImageCollection::robustModelEstimation(
                geometricMatches, &sfmData, regionPerView,
                GeometricFilterMatrix_HGrowing(geometricErrorMax, maxIteration), mapPutativesMatches, guidedMatching);
        }
        break;
    }
    ...
    for(const auto& matchGeo : geometricMatches)
        ALICEVISION_LOG_INFO("\t- image pair (" + std::to_string(matchGeo.first.first) + ", " +
                             std::to_string(matchGeo.first.second) + ") contains " +
                             std::to_string(matchGeo.second.getNbAllMatches()) + " geometric matches.");
...

    {
        for(const auto& geometricMatch : geometricMatches)
        {
            ...
            for(const auto& match : matchesPerDesc)
            {
                ...
                if(rRegions && lRegions)
                {
                    ...
                    // std::cout << "Left features: " << lRegions->Features().size() << ", right features: " <<
                    // rRegions->Features().size() << ", num matches: " << inputMatches.size() << ", num filtered
                    // matches: " << outMatches.size() << std::endl;
        ...
        for(const auto& matchGridFiltering : finalMatches)
            ALICEVISION_LOG_INFO("\t- image pair (" + std::to_string(matchGridFiltering.first.first) + ", " +
                                 std::to_string(matchGridFiltering.first.second) + ") contains " +
                                 std::to_string(matchGridFiltering.second.getNbAllMatches()) + " geometric matches.");
    }
    ...
    if(exportDebugFiles)
    {
        ...
        PairwiseMatchingToAdjacencyMatrixSVG(sfmData.getViews().size(), finalMatches,
                                             (fs::path(matchesFolder) / "GeometricAdjacencyMatrix.svg").string());
